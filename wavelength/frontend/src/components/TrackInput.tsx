...
interface Props {
  ...
  jamUrl?: string;
  onSave: (track: Track | null, jamUrl?: string) => void;
  onClose: () => void;
  accentColor: string;
}

export default function TrackInput({ currentTrack, jamUrl: initJamUrl = '', onSave, onClose, accentColor }: Props) {
  ...
  useEffect(() => {
    if (currentTrack?.source !== 'manual') setUrl(currentTrack?.url ?? '');
    else { setMode('manual'); setArtist(currentTrack.artist ?? ''); setTitle(currentTrack.title ?? ''); }
  }, [currentTrack]);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (src === 'youtube') { const id = getYtId(url); if (id) albumArt = `https://img.youtube.com/vi/${id}/mqdefault.jpg`; }
  ...
  function handleSave() {
    ...
    if (mode === 'url' && preview) track = preview;
    else if (mode === 'manual' && title.trim()) track = { title: title.trim(), artist: artist.trim() || undefined, source: 'manual' };
    onSave(track, jamUrl.trim() || undefined);
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in"
      ...
      <div className="glass-strong rounded-3xl w-full max-w-md p-6 animate-slide-up overflow-y-auto" style={{ maxHeight: '92vh' }}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-white">Qu'est-ce que tu écoutes ?</h2>
          <button onClick={onClose} className="p-1.5 rounded-xl hover:bg-white/10 transition-colors"><X className="w-4 h-4 text-white/60" /></button>
        </div>
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-4" style={{ background: 'rgba(255,255,255,0.05)' }}>
          {(['url', 'manual'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all ${mode === m ? 'text-white' : 'text-white/40 hover:text-white/60'}`}
        ...
        {mode === 'url' ? (
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center gap-1.5 text-xs text-white/30"><SpotifyLogo size={15} /><span>Spotify</span></div>
              <div className="flex items-center gap-1.5 text-xs text-white/30"><YouTubeLogo size={15} /><span>YouTube</span></div>
            </div>
            <div className="relative mb-4">
              <Link2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30 pointer-events-none" />
              <input className="wl-input w-full rounded-xl pl-10 pr-10 py-3 text-sm" placeholder="Colle ton lien ici…"
              ...
              {!loading && url && <button onClick={() => { setUrl(''); setPreview(null); }}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-0.5 rounded-full hover:bg-white/10"><X className="w-3.5 h-3.5 text-white/40" /></button>}
            </div>
        ...
        {/* Spotify Jam separator */}
        <div className="my-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.07)' }} />

        {/* Jam URL */}
        <div className="mb-5">
          <label className="text-xs font-semibold uppercase tracking-wider mb-2 flex items-center gap-1.5" style={{ color: '#1DB954' }}>
            <SpotifyLogo size={13} /> Jam Spotify <span className="text-white/25 normal-case font-normal">(optionnel)</span>
          </label>
          <div className="relative">
            <SpotifyLogo size={16} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input className="wl-input w-full rounded-xl pl-9 pr-4 py-2.5 text-sm"
              placeholder="open.spotify.com/jam/… ou lien partagé"
              value={jamUrl} onChange={e => setJamUrl(e.target.value)} />
          </div>
          <p className="text-xs text-white/25 mt-1 pl-1">Partage un Jam actif — les gens à proximité pourront le rejoindre</p>
        </div>

        <div className="flex gap-2">
          {currentTrack && (
            <button onClick={() => { onSave(null, ''); onClose(); }}
              className="py-3 px-4 rounded-xl text-xs font-semibold text-red-400 border border-red-400/20 hover:bg-red-400/10 transition-colors">
              Arrêter
            </button>
          )}
          <button onClick={handleSave} disabled={!canSave && !jamUrl.trim()}
            className="flex-1 py-3 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed active:scale-95 flex items-center justify-center gap-2"
    ...
    <div className="rounded-2xl overflow-hidden mb-4 animate-pop-in"
      ...
      <div className="flex gap-3 p-3">
        {track.albumArt
          ...
          : <div className="w-14 h-14 rounded-xl flex-shrink-0 flex items-center justify-center text-2xl" style={{ background: 'rgba(255,255,255,0.06)' }}>🎵</div>}
